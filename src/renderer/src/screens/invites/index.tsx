import { Invite, ServerError } from '@renderer/@types/subdivision'
...
const InvitesPage = () => {
  ...
  const { data, isError, isLoading, isSuccess, error } = useQuery<Invite[], ServerError>({
          ...
          <div className="grid mobile:grid-cols-1 notebook:grid-cols-2 gap-10 ">
            <div className="p-10 flex flex-col gap-5 bg-[#0F1623] rounded">
              <div className="w-full flex items-center justify-between">
                <span className="text-xl font-bold">Заявка #{item.user.user_id}</span>
                <div className="flex items-center gap-5">
                  <IconSpace title="Принять" onClick={() => mutateInvite(item)}>
                    <Check size={12} />
                  </IconSpace>
                  <IconSpace title="Отклонить" onClick={() => mutateDelete(item)}>
                    <X size={12} />
                  </IconSpace>
                </div>
              </div>
              <div>
                <span className="opacity-70">
                  Пользователь <strong className="text-blue-800">{item.user.username}</strong> хочет
                  присоединиться к отделу {item.subdivision.subdivision_name}
                </span>
              </div>
            </div>
          </div>
